# Estructura de Contenido: Sección Pricing

> Implementado en `src/components/landing/PricingSection.tsx` en la landing `/`. Los botones enlazan a `/login`; no hay billing integrado aún (ver [08 · Roadmap](./08-roadmap.md)).

## Bloque 1: Título y Subtítulo
- **Título:** Invierte en el rendimiento de tu terapia
- **Subtítulo:** Si ya inviertes en tu salud mental, haz que cada sesión cuente el doble. Llega a consulta con el trabajo hecho, por menos de lo que cuesta un café a la semana.

## Bloque 2: Tarjeta Plan Mensual (Izquierda)
- **Etiqueta superior:** Para procesos cortos
- **Precio principal:** 9,99 € / mes
- **Características:**
    - Acceso ilimitado al chat de IA cognitivo-conductual.
    - Diario inteligente y registro de pensamientos.
    - Exportación ilimitada de informes en PDF.
    - Sin permanencia, cancela cuando quieras.
- **Texto de descuento:** ¿Tienes el código de tu terapeuta? Consíguelo por 7,99 € / mes para siempre.
- **Botón:** Empezar ahora

## Bloque 3: Tarjeta Plan Anual (Derecha / Destacada)
- **Etiqueta superior:** El precio de una sesión
- **Precio principal:** 59,99 € / año
- **Características:**
    - Incluye todo lo del Plan Mensual.
    - Equivale a solo 4,99 € al mes.
    - Un único pago anual.
    - Ahorra un 50% frente al pago mensual.
- **Texto de descuento:** ¿Tienes el código de tu terapeuta? Consíguelo por 49,99 € / año para siempre.
- **Botón:** Optimizar mi terapia

## Bloque 4: Pie de página
- **Texto de cierre:** 🔒 Privacidad de nivel clínico. Tus datos están completamente cifrados y tú mantienes el control total de lo que compartes.